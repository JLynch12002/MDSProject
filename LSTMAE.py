...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, dropout):
        ...
        # Project bidirectional output back to original hidden size
        self.projection = nn.Linear(hidden_size * 2, hidden_size)
        
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        out, (last_hidden_state, last_cell_state) = self.lstm_encode(x) # out shape (batch_size, seq_len, hidden_size * 2) due to bidirectional
        
        # Apply attention to focus on important timesteps
        attention_scores = self.attention(out) 
        attention_weights = torch.softmax(attention_scores, dim=1) 
        
        # Apply attention weights to encoded sequence
        attended_out = out * attention_weights  # (batch_size, seq_len, hidden_size * 2)
        
        # Project back to original hidden size
        projected_out = self.projection(attended_out)  
        
        # Takes last hidden state as latent representation
...
class Decoder(nn.Module):
    def __init__(self, input_size, hidden_size, dropout):
        ...
        # takes repeated hidden states as input
